				...
				<div className='p-4 mt-4 flex flex-col gap-8 w-full'>
					{!state.provider.value ? (
						<article className='items-center flex flex-col gap-4 py-8'>
							<Greeting isReturningUser={Boolean(state.bundle.value)} />
							<Button
...
const Greeting = ({ isReturningUser }: { isReturningUser: boolean }) => {
	if (isReturningUser) return (
		<>
			<h2 class='text-2xl font-bold'>Welcome Back</h2>
			<p class='text-gray-500'>Connect your wallet to continue where you previously left off.</p>
		</>
	)
	...
	return (
		<>
			...
			<p class='text-gray-500'>Let's start by connecting your wallet.</p>
		</>
	)
}
