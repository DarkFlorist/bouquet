...
function fetchBundleFromStorage() {
...
function fetchSettingsFromStorage() {
    ...
    const defaultValues = { blocksInFuture: 3n, priorityFee: 10n ** 9n * 3n, simulationRelayEndpoint: defaultNetwork.simulationRelay, submissionRelayEndpoint: defaultNetwork.submissionRelay };
    ...
    if (!custom) {
        ...
        try {
            ...
            if ('simulationRelayEndpoint' in parsed)
                defaultValues.simulationRelayEndpoint = parsed.simulationRelayEndpoint;
            if ('submissionRelayEndpoint' in parsed)
                defaultValues.submissionRelayEndpoint = parsed.submissionRelayEndpoint;
            if ('priorityFee' in parsed)
                defaultValues.priorityFee = BigInt(parsed.priorityFee);
            if ('blocksInFuture' in parsed)
                defaultValues.blocksInFuture = BigInt(parsed.blocksInFuture);
            return defaultValues;
        }
        catch {
            return defaultValues;
