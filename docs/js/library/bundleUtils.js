import { getAddress, getNumber, Signature, verifyAuthorization } from 'ethers';
...
export const getMaxBaseFeeInFutureBlock = (baseFee, blocksInFuture) => {
    ...
    for (let block = 0n; block < blocksInFuture; block++)
        maxBaseFee = (maxBaseFee * 1125n) / 1000n + 1n;
    return maxBaseFee;
};
export const getFutureFeeProjection = (blockInfo, feeSettings) => {
...
export const withPriorityFee = (blockInfo, priorityFee) => ({ ...blockInfo, priorityFee });
async function requestSimulatedCountsOnNetwork(provider) {
    const { payload } = await provider.send('interceptor_getSimulationStack', ['1.0.1']);
...
export const getRawTransactionsAndCalculateFeesAndNonces = async (bundle, signers, provider, blockInfo, blocksInFuture, fundingAmountMin, maxBaseFee) => {
    const transactions = [];
    const unsignedAuthorizationAuthorities = [...new Set(bundle.transactions
            .filter(isClearDelegationTransaction)
            .flatMap((transaction) => (transaction.authorizationList ?? []).flatMap((authorization) => authorization.authority === undefined || authorization.r !== undefined ? [] : [getAddress(addressString(authorization.authority))])))];
    ...
    const accNonces = {};
    const getNonceBeforeSimulation = async (address) => {
        const normalizedAddress = getAddress(address);
        if (normalizedAddress in accNonces)
            return accNonces[normalizedAddress];
        const transactionCount = await provider.getTransactionCount(normalizedAddress, 'latest') - (inSimulation[normalizedAddress] ?? 0);
        if (transactionCount < 0)
            throw new Error('Interceptor returned an invalid nonce for this simulation stack.');
        accNonces[normalizedAddress] = transactionCount;
        return transactionCount;
    };
    const authorizationNonces = {};
    for (const authority of unsignedAuthorizationAuthorities)
        authorizationNonces[authority] = BigInt(await getNonceBeforeSimulation(authority));
...
export const createBundleTransactions = async (bundle, signers, blockInfo, blocksInFuture, fundingAmountMin, authorizationNonces) => {
    ...
    return Promise.all(bundle.transactions.map(async (bundleTransaction) => {
        ...
        if (from === 'FUNDING' && !isDelegationClear) {
            ...
            if (fundingValue < 0n)
                throw new Error('Funding wallet balance requirement is smaller than its transaction fees');
            return {
            ...
            if (!signer)
                throw new Error(from === 'FUNDING' ? 'No burner wallet provided' : `No signer provided for ${addressString(from)}`);
            const resolvedAuthorizations = [];
            for (const authorization of authorizationList ?? []) {
                ...
                if (authorization.authority === undefined)
                    throw new Error('Unsigned authorization is missing its authority');
                const authorityAddress = addressString(authorization.authority);
                if (isDelegationClear && authorityAddress === signer.address)
                    throw new Error('The funding wallet must be different from the compromised account');
                const authoritySigner = signers.bundleSigners[authorityAddress];
                if (!authoritySigner)
                    throw new Error(`No signer provided for authorization authority ${addressString(authorization.authority)}`);
                ...
                if (authorizationNonce === undefined)
                    throw new Error(`Missing current authorization nonce for ${authorityAddress}`);
