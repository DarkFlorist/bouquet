...
import { EthSimulateV1Params, EthSimulateV1Result, JsonRpcResponse, TransactionType } from '../types/ethSimulateTypes.js';
import { serialize } from '../types/ethereumTypes.js';
import { addressString, min, hexStringToUint8Array } from './utils.js';
const isRecord = (value) => typeof value === 'object' && value !== null;
export const createRelaySimulationPayload = (transactions, targetBlock) => JSON.stringify({
    jsonrpc: '2.0',
    id: 0,
    method: 'eth_callBundle',
    params: [{ txs: transactions, blockNumber: `0x${targetBlock.toString(16)}`, stateBlockNumber: 'latest' }],
});
export async function simulateBundle(bundle, fundingAmountMin, provider, signers, blockInfo, targetBlock, network) {
    const blocksInFuture = targetBlock - blockInfo.blockNumber;
    if (blocksInFuture <= 0n)
        throw new Error('Simulation target block must be in the future');
    ...
    if (bundle.rescueMode && network.relayMode !== 'relay')
        throw new Error('EIP-7702 rescue bundles require a private relay');
    ...
    const bigIntify = (ethersValue) => ethersValue ? BigInt(ethersValue.toString()) : undefined;
    switch (network.relayMode) {
        case 'mempool': {
            if (network.mempoolSimulationRpcEndpoint === undefined)
                throw new Error('mempoolSimulationRpcEndpoint is not defined');
            const data = {
            ...
            try {
                ...
                return {
                    ...
                    gasFees: txs.reduce((totalFee, tx, currentIndex) => {
                        if (tx.transaction.gasPrice)
                            return totalFee + BigInt(tx.transaction.gasPrice) * calls[currentIndex].gasUsed;
                ...
                if (error instanceof DOMException && error.name === 'TimeoutError')
                    throw new Error(`Simulation timed out to RPC: ${network.mempoolSimulationRpcEndpoint}`);
                throw error;
            }
        }
        case 'relay': {
            if (network.simulationRelayEndpoint === undefined)
                throw new Error('simulationRelayEndpoint is not defined');
            ...
            const request = await fetch(network.simulationRelayEndpoint, { method: 'POST', body: payload, headers: { 'Content-Type': 'application/json', 'X-Flashbots-Signature': flashbotsSig } });
            ...
            return {
                ...
                totalGasUsed: callResult.results.reduce((a, b) => a + BigInt(b.gasUsed), 0n),
...
let bundleId = 1;
export async function sendBundle(bundle, targetBlocks, fundingAmountMin, provider, signers, blockInfo, network) {
    if (targetBlocks.length === 0)
        throw new Error('At least one target block is required');
    const blocksInFuture = targetBlocks.reduce((largestDistance, targetBlock) => {
        const distance = targetBlock - blockInfo.blockNumber;
        if (distance <= 0n)
            throw new Error('Bundle target blocks must be in the future');
    ...
    const signingBlockInfo = withPriorityFee(blockInfo, network.priorityFee);
    const maxBaseFee = getMaxBaseFeeInFutureBlock(blockInfo.baseFee, blocksInFuture);
    const transactions = (await getRawTransactionsAndCalculateFeesAndNonces(bundle, signers, provider.provider, signingBlockInfo, blocksInFuture, fundingAmountMin, maxBaseFee)).map((x) => x.rawTransaction);
    switch (network.relayMode) {
        case 'mempool': {
            ...
            if (network.mempoolSubmitRpcEndpoint === undefined)
                throw new Error('MemPool Submit RPC Endpoint is not set');
        ...
        case 'relay': {
            if (network.submissionRelayEndpoint === undefined)
                throw new Error('submissionRelayEndpoint is not defined');
            const submissionRelayEndpoint = network.submissionRelayEndpoint;
            const submissionResults = await Promise.all(targetBlocks.map(async (targetBlock) => {
                try {
                    ...
                    const flashbotsSig = `${await provider.authSigner.getAddress()}:${await provider.authSigner.signMessage(id(payload))}`;
                    const request = await fetch(submissionRelayEndpoint, { method: 'POST', body: payload, headers: { 'Content-Type': 'application/json', 'X-Flashbots-Signature': flashbotsSig } });
                    const response = await request.json();
                    if (!isRecord(response))
                        throw new Error('Relay returned an invalid bundle submission response');
                    if (isRecord(response.error) && typeof response.error.message === 'string')
                        throw new Error(response.error.message);
                    if (!isRecord(response.result) || typeof response.result.bundleHash !== 'string')
                        throw new Error('Relay did not return a bundle hash');
...
export async function checkBundleInclusion(transactions, provider) {
    ...
    const includedInBlocks = Array.from(new Set(receipts.filter((receipt) => receipt !== null).map((receipt) => BigInt(receipt.blockNumber))));
