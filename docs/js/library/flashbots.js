...
export async function simulateBundle(bundle, fundingAmountMin, provider, signers, blockInfo, network) {
    if (network.blocksInFuture <= 0n)
...
let bundleId = 1;
export async function sendBundle(bundle, targetBlock, fundingAmountMin, provider, signers, blockInfo, network) {
    if (network.blocksInFuture <= 0n)
        throw new Error('Blocks in future is negative or zero');
    const maxBaseFee = getMaxBaseFeeInFutureBlock(blockInfo.baseFee, network.blocksInFuture);
    const transactions = (await getRawTransactionsAndCalculateFeesAndNonces(createBundleTransactions(bundle, signers, blockInfo, network.blocksInFuture, fundingAmountMin), provider.provider, blockInfo, maxBaseFee)).map((x) => x.rawTransaction);
    switch (network.relayMode) {
        case 'mempool': {
        ...
        case 'relay': {
            ...
            const request = await fetch(network.submissionRelayEndpoint, { method: 'POST', body: payload, headers: { 'Content-Type': 'application/json', 'X-Flashbots-Signature': flashbotsSig } });
            const response = await request.json();
            if (response.error !== undefined && response.error !== null) {
                throw {
                    message: response.error.message,
