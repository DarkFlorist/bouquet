...
export const Bundles = ({ outstandingBundles, bouquetNetwork, blockInfo, }) => {
    ...
    return (_jsx("div", { class: 'flex flex-col gap-3', children: outstandingBundles.value.success
            ...
            : _jsxs(_Fragment, { children: [outstandingBundles.value.missedTargets.slice(-5).map(({ targetBlock, diagnostic }) => _jsxs("p", { class: 'text-sm text-white/60', children: ["Bundle for block ", targetBlock.toString(), " was not included. ", diagnostic] }, targetBlock.toString())), latestPendingBundle === undefined ? null : _jsxs("div", { class: 'flex items-center gap-2 text-white', children: [_jsxs("svg", { class: 'animate-spin h-4 w-4 text-white', xmlns: 'http://www.w3.org/2000/svg', fill: 'none', viewBox: '0 0 24 24', children: [_jsx("circle", { class: 'opacity-25', cx: '12', cy: '12', r: '10', stroke: 'currentColor', "stroke-width": '4' }), _jsx("path", { class: 'opacity-75', fill: 'currentColor', d: 'M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z' })] }), _jsxs("p", { children: [describeBundleTarget(blockInfo.value.blockNumber, latestPendingBundle.targetBlock), " Max fee: ", Number(formatUnits(latestPendingBundle.gas.baseFee + latestPendingBundle.gas.priorityFee, 'gwei')).toPrecision(3), " gwei per gas."] })] })] }) }));
};
export const Submit = ({ provider, bundle, fundingAmountMin, signers, bouquetSettings, blockInfo, }) => {
    ...
    const missingRequirements = useComputed(() => {
        ...
        if (bundle.value.rescueMode && bouquetNetwork.value.relayMode !== 'relay')
            return 'EIP-7702 rescue bundles require a private relay network.';
    ...
    async function simulateCallback() {
        ...
        const simulationResult = await simulateBundle(bundle.value, fundingAmountMin.peek(), provider.value, signers.peek(), blockInfo.peek(), targetBlock, network);
    ...
    async function runBundleSubmission(blockNumber) {
        ...
        if (included.length > 0) {
            ...
            if (submissionStatus.value.active && !outstandingBundles.value.success) {
                ...
                try {
                    ...
                    const bundleRequest = await sendBundle(bundle.value, targetBlocks, fundingAmountMin.peek(), provider.value, signers.peek(), blockInfo.peek(), bouquetNetwork.peek());
                    ...
                    for (const submission of bundleRequest.submissions) {
                        ...
                        if (!(attemptIdentifier in nextBundles))
                            nextBundles[attemptIdentifier] = { bundleHash: submission.bundleIdentifier, targetBlock: submission.targetBlock, gas, transactions: bundleRequest.bundleTransactions, included: false };
                    }
    ...
    return (_jsxs(_Fragment, { children: [_jsxs("h2", { className: 'font-bold text-2xl', children: [_jsx("span", { class: 'text-gray-500', children: "3." }), " Submit"] }), _jsx(SettingsModal, { display: showSettings, bouquetNetwork: bouquetNetwork, bouquetSettings: bouquetSettings }), !outstandingBundles.value.success && missingRequirements.value ? (_jsx("p", { children: missingRequirements.peek() })) : (_jsxs("div", { className: 'flex flex-col w-full gap-4', children: [_jsxs("div", { children: [_jsxs("p", { children: [_jsx("span", { className: 'font-bold', children: "Current block:" }), " ", blockInfo.value.blockNumber.toString()] }), bouquetNetwork.value.relayMode === 'mempool' ? _jsxs(_Fragment, { children: [_jsx("div", { style: 'padding-bottom: 10px;', children: _jsx(SingleNotice, { variant: 'warn', title: 'Mempool mode is dangerous', description: `You are currently using Mempool mode. Transactions are sent individually so some transactions may not make it onto the blockchain. This mode should only be used if a priate relay is unavailable for the network. Additionally, if a sweeper is active on your account there is a high risk that rescue attempts may fail, allowing the sweeper to steal your gas funds and other assets. Use this mode only as a last resort when no other options are available.` }) }), _jsxs("p", { children: [_jsx("span", { className: 'font-bold', children: "Gas:" }), " ", formatUnits(futureFeeProjection.value.baseFee, 'gwei'), " gwei + ", formatUnits(futureFeeProjection.value.priorityFee, 'gwei'), " gwei priority"] }), _jsxs("p", { children: [_jsx("span", { className: 'font-bold', children: "Transaction Submit RPC:" }), " ", bouquetNetwork.value.mempoolSubmitRpcEndpoint] }), _jsxs("p", { children: [_jsx("span", { className: 'font-bold', children: "Transaction Simulation RPC:" }), " ", bouquetNetwork.value.mempoolSimulationRpcEndpoint] })] }) : _jsxs(_Fragment, { children: [_jsxs("p", { children: [_jsx("span", { className: 'font-bold', children: "Gas:" }), " ", formatUnits(futureFeeProjection.value.baseFee, 'gwei'), " gwei + ", formatUnits(futureFeeProjection.value.priorityFee, 'gwei'), " gwei priority"] }), _jsxs("div", { class: 'grid gap-2 py-2 md:grid-cols-2', children: [_jsxs("div", { class: 'flex min-w-0 flex-col gap-1 border border-white/20 bg-white/5 p-3', children: [_jsx("span", { class: 'text-xs font-semibold uppercase tracking-wide text-white/50', children: "Simulation relay" }), _jsx("span", { class: 'break-all font-mono text-xs text-white/80', children: bouquetNetwork.value.simulationRelayEndpoint })] }), _jsxs("div", { class: 'flex min-w-0 flex-col gap-1 border border-white/20 bg-white/5 p-3', children: [_jsx("span", { class: 'text-xs font-semibold uppercase tracking-wide text-white/50', children: "Submission relay" }), _jsx("span", { class: 'break-all font-mono text-xs text-white/80', children: bouquetNetwork.value.submissionRelayEndpoint })] })] }), _jsxs("p", { children: ["Transactions will be attempt to be included in the block ", bouquetNetwork.value.blocksInFuture.toString(), " blocks from current block."] })] }), _jsxs("p", { children: ["You can edit these settings ", _jsx("button", { className: 'font-bold underline', onClick: () => showSettings.value = true, children: "here" }), "."] })] }), _jsxs("div", { className: 'flex flex-row gap-6', children: [_jsx(Button, { onClick: () => waitForSimulation(simulateCallback), disabled: simulationPromise.value.state === 'pending', variant: 'secondary', children: "Simulate" }), _jsx(Button, { onClick: toggleSubmission, children: submissionStatus.value.active ? (bouquetNetwork.value.relayMode === 'relay' ? `Stop submitting to relay` : `Stop tracking the transactions`) : (bouquetNetwork.value.relayMode === 'mempool' ? `Accept the Risks and Submit` : `Submit to ${bouquetNetwork.value.relayMode}`) })] }), _jsx(SimulationResult, { state: simulationPromise }), _jsx(Bundles, { outstandingBundles: outstandingBundles, bouquetNetwork: bouquetNetwork, blockInfo: blockInfo })] }))] }));
};
