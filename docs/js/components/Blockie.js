...
class Future {
    ...
    get asPromise() { return this.promise; }
}
function addressString(address) {
    return `0x${address.toString(16).padStart(40, '0')}`;
}
function generateIdenticon(address, scale, canvasRef) {
...
async function renderBlockieToUrl(address, scale) {
...
export function Blockie(props) {
    ...
    return _jsx("img", { src: dataURL.value.state !== 'resolved' ? 'data:image/gif;base64,R0lGODlhAQABAAAAACwAAAAAAQABAAA=' : dataURL.value.value, style: {
            ...props.style,
            width: `${dimension}px`,
            height: `${dimension}px`,
