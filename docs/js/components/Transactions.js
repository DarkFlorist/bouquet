...
export const Transactions = ({ provider, bundle, blockInfo, appSettings, signers }) => {
    ...
    async function fetchAbis() {
        ...
        try {
            ...
            const requests = await Promise.all(uniqueAddresses.map((address) => fetch(`${network.blockExplorerApi}/api?module=contract&action=getsourcecode&address=${getAddress(address.toLowerCase())}&apiKey=PSW8C433Q667DVEX5BCRMGNAH9FSGFZ7Q8`)));
